...
DROP FUNCTION IF EXISTS public.update_predictions_for_program() CASCADE;
...
BEGIN
  ...
  -- Limit to 0-100 range and round to 2 decimal places
  RETURN GREATEST(0, LEAST(100, ROUND((100 - (ABS(predicted - actual) / actual * 100))::numeric, 2)));
END;
$$ LANGUAGE plpgsql;

-- Improved function to calculate prediction score
CREATE OR REPLACE FUNCTION calculate_prediction_score(
  accuracy numeric
) RETURNS integer AS $$
BEGIN
  -- Convert accuracy percentage directly to points
  -- 90-100% accuracy = 100 points
  -- 80-89% accuracy = 80 points
  -- 70-79% accuracy = 60 points
  -- 60-69% accuracy = 40 points
  -- 50-59% accuracy = 20 points
  -- Below 50% = 0 points
  RETURN CASE
...
-- Update existing predictions to recalculate scores
DO $$
BEGIN
