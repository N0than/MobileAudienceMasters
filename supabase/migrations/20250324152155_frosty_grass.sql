/*
  # Update prediction accuracy calculation and scoring system

  1. Changes
    - Update accuracy calculation to use (1 - |predicted - actual|/actual) * 100
    - Update scoring system with new tiers
    - Add trigger to recalculate scores when predictions change
...
BEGIN
  ...
  -- Calculate accuracy using formula: (1 - |predicted - actual|/actual) * 100
  -- Round to 1 decimal place and limit to 0-100 range
  RETURN GREATEST(0, LEAST(100, ROUND(((1 - ABS(predicted - actual) / actual) * 100)::numeric, 1)));
END;
$$ LANGUAGE plpgsql;
