/*
  ...
  1. Changes
    - Update accuracy calculation to handle overestimation differently
    - Update scoring system with new point tiers
...
-- Function to calculate prediction accuracy with different handling for overestimation
...
BEGIN
  ...
  -- Calculate accuracy with different handling for overestimation
  RETURN CASE
    WHEN predicted <= actual THEN ROUND((predicted / actual * 100)::numeric, 1)
    ELSE ROUND((actual / predicted * 100)::numeric, 1)  -- Penalize overestimation
  END;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  RETURN CASE
    ...
    WHEN accuracy >= 80 THEN 50   -- 80-84.99%: 50 points
    WHEN accuracy >= 70 THEN 30   -- 70-79.99%: 30 points
    WHEN accuracy >= 60 THEN 20   -- 60-69.99%: 20 points
    WHEN accuracy >= 50 THEN 10   -- 50-59.99%: 10 points
    ELSE 0                        -- < 50%: 0 points
  END;
END;
$$ LANGUAGE plpgsql;
